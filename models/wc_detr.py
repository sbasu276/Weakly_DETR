...
class DETRMiL(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries, device, lambda_gt,
                 alpha, refiner_depth=3, aux_loss=False):
        ...
        self.refiner = Refiner(hidden_dim, num_classes, refiner_depth, device, lambda_gt, alpha)
...
class Refiner(nn.Module):
    def __init__(self, hidden_dim, num_classes, refiner_depth, device, lambda_gt, alpha):
        super().__init__()
        ...
        self.refinement_agents = RefinementAgents(hidden_dim, num_classes, refiner_depth)
    ...
    def forward(self, x, boxes, labels):
        with torch.set_grad_enabled(self.training):
            #im_labels = F.one_hot(labels, num_classes=2)
            ...
            if self.training:
                ...
                loss_mil = self.mil_loss(im_cls_score, labels)
                ...
                for i, prop_score in enumerate(refine_score):
                    ...
                    loss = loss + refine_loss.mean()
                # pytorch0.4 bug on gathering scalar(0-dim) tensors
                #for k, v in return_dict['losses'].items():
                #    return_dict['losses'][k] = v.unsqueeze(0)
                return_dict['loss'] = self.alpha*loss_mil + (1-self.alpha)*loss
...
def batch_box_iou(boxes_1, boxes_2):
    """
    Args:
    ...
    """
    boxes1 = box_ops.box_cxcywh_to_xyxy(boxes_1)
    boxes2 = box_ops.box_cxcywh_to_xyxy(boxes_2)
...
def select_proposals(boxes, cls_prob, im_labels, device, lambda_gt=0.5, bg_sep=False):
...
class RefineLoss(nn.Module):
    def forward(self, prop_score, label_score, cls_weights):
        ...
        loss = prop_score * label_score #*cls_weights
